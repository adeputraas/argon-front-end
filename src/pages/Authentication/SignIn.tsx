...
const SignIn: React.FC = () => {
  ...
  const onLoggedInClick = async () => {
    if(userName && passwordUser){
      const dtoUser = { username: userName, password: passwordUser};
      const loginResp = await API.post('USERS', 'signin', dtoUser);
      if(loginResp.status === "Bad Request") {
        toast.error(loginResp.message, {
          position: "top-right"
        });
      } else {
        localStorage.setItem("user", JSON.stringify(loginResp.data.detailUser));
